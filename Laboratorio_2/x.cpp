...
void mostrarEstado(Pila** pilas, int m) {
    ...
    for (int tope_mas_alto = altura_maxima - 1; tope_mas_alto >= 0; tope_mas_alto--) { //desde el tope más alto bajamos
...
void retirarContenedor(Pila** pilas, int m, int valor) {
    ...
    for (int i = 0; i < m; i++) { 
        valor_buscado = pilas[i]->buscar(valor);
        if (valor_buscado != -1) { 
            numero_pila = i; //si encontramos el número, definimos la pila donde está
            break; 
        }
    }
    ...
    Pila* p = pilas[numero_pila];

    while (!p->pila_vacia() && p->cima() != valor) {
        int encima = p->pop(); //último término
        cout << "Moviendo contenedor " << encima << endl;

        bool colocado = false;
        for (int j = 0; j < m; j++) {
            if (j != numero_pila && !pilas[j]->pila_llena()) { //si no está llena la pila y no es la misma donde estamos
